...
export class SalesforceSnapshotService {
  ...
  private async storeSnapshot(
    ...
    try {
      ...
      console.log(
        `Successfully stored ${records.length} ${metadata.entityType} records in snapshot ${metadata.snapshotId}`
      );
    } catch (error) {
  ...
  async scheduleSnapshots(
    ...
    console.log(
      `Scheduled Salesforce snapshots for organization ${organizationId} every ${intervalHours} hours`
    );
    ...
    setInterval(async () => {
      try {
        ...
        const lastOpportunitySnapshot = await this.getLastSnapshotTimestamp(
          organizationId,
          "opportunity"
        );
        const lastContactSnapshot = await this.getLastSnapshotTimestamp(
          organizationId,
          "contact"
        );
        const lastAccountSnapshot = await this.getLastSnapshotTimestamp(
          organizationId,
          "account"
        );
        ...
        console.log(
          `Completed scheduled snapshot for organization ${organizationId}`
        );
      } catch (error) {
        console.error(
          `Error in scheduled snapshot for organization ${organizationId}:`,
          error
        );
      }
 ...
 */
export async function initializeR2Bucket(
  s3Client: S3Client,
  bucketName: string
): Promise<void> {
  try {
    ...
    if ((error as any).name === "NoSuchBucket") {
      console.log(
        `R2 bucket '${bucketName}' doesn't exist, please create it in the Cloudflare dashboard`
      );
    } else {
